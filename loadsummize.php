<?php
    ...
    $url = "http://search.twitter.com/search.atom?q=%40wants"; // summize search
...
// create a new cURL resource
    ...
    foreach ($feed->children() as $entry) {
        ...
        $text = $db->escape($entry->title); // use the title, not the linked text
        ...
        // only use if it doesn't start with @wants (loadtwitter.php covers those)
        ...
        foreach ($entry->children() as $entry2) {
            ...
            if ($attributes[0] == "image/png") {
                $user_profile_image_url = $attributes[2];
            }
        }
